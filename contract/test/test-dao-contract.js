...
const contractPath = myRequire.resolve(`../src/dao.contract.js`);
...
test('Start the DAO contract and test joining', async t => {
  ...
  const createProposalInvitation = E(publicFacet).createProposalInvitation(
  ...
  const createProposalSeat = await E(zoe).offer(
  ...
  t.truthy(
    payoutMembership,
    'User should receive a Membership NFT upon joining',
  );
  ...
  /** @type {[string, bigint][]} */
  const currentMembershipDetails = [['MembershipCard1', 1n]];

  /** @type {[string, bigint][]} */
  const updatedMembershipDetails = [['MembershipCard2', 1n]];
  ...
  const currentMembership = AmountMath.make(
    membershipBrand,
    currentMembershipBag,
  );
  const updatedMembership = AmountMath.make(
    membershipBrand,
    updatedMembershipBag,
  );
  const daoTokenPurse = daoTokenIssuer.makeEmptyPurse();
  daoTokenPurse.deposit(payoutDaoTokens); // dposit existing DAO tokens
  const daoTokensWithdrawn = await E(daoTokenPurse).withdraw(
    AmountMath.make(daoTokenBrand, requiredDaoTokens),
  );
  ...
  const membershipWithdrawal =
    await E(membershipPurse).withdraw(currentMembership);
  ...
  const voteSeat = await E(zoe).offer(
    voteInvitation,
    voteOffer,
    {
  ...
  t.is(
    voteResult,
    'Vote accepted. 10 DaoTokens spent.',
    'User with Membership NFT and DAO tokens should be able to vote',
  );
  t.truthy(
    votePayoutMembership,
    'User should receive back the Membership NFT after voting',
  );
  const votePayoutMembershipAmount =
    await E(membershipIssuer).getAmountOf(votePayoutMembership);
  t.is(
    votePayoutMembershipAmount.value.payload[0][0],
    'MembershipCard2',
