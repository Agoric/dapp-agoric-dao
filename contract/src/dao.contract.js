...
export const start = async (zcf, privateArgs, baggage) => {
    ...
    atomicRearrange(
      ...
      harden([
        ...
        [
          membershipSeat,
          joiningmMemberSeat,
          { NewMembership: membershipAmount },
        ],
  ...
  const vote = async (voterSeat, { proposalId, voteFor }) => {
    const membershipHeld = voterSeat.getAmountAllocated(
      'Membership',
      membershipBrand,
    );
    ...
    /** @type {[string, bigint][]} */
    const choices = [['MembershipCard2', 1n]];
  ...
  const createProposalInvitation = () =>
    zcf.makeInvitation(
      createProposal,
      'Create DAO Proposal',
      undefined,
      createProposalProposalShape,
    );
